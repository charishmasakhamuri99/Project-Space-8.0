module control_unit #(
  ...
  always_comb begin
    ...
    case (state)
      ...
      S_COMPUTE   : if (compute_col == OUTPUT_WIDTH-1)               next_state = S_ROW_DONE;
      S_ROW_DONE  : if (buf_empty) begin
                      if (compute_row == OUTPUT_WIDTH-1)             next_state = S_FRAME_DONE;
                      else                                           next_state = S_FILL_WIN;
                    end
  ...
  always_ff @(posedge clk or negedge rst) begin
    if (!rst) begin
      ...
      case (state)
        ...
        S_FILL_WIN: begin
          shift_en   <= 1'b1;
          lb_rd_addr <= compute_col[$clog2(IMAGE_WIDTH)-1:0];
        end

        S_COMPUTE: begin
          shift_en       <= 1'b1;
          pe_pixel_valid <= window_valid;
          lb_rd_addr     <= compute_col[$clog2(IMAGE_WIDTH)-1:0] + 1'b1;
